...
(function($){
	...
	$.concrete('ss.promospots', function($){
	    ...
	    **********************************/
		...
		$('#AvailableItemsFilter').concrete({
			...
			load: function(pageid, template) {
				...
				this.setPageID(pageid); this.setTemplate(template);
				
				var self = this;
				this.concrete('.').load(promo_url+'/AvailableItemsFilterFields/'+pageid+'/'+template, function(){ self.search() });
			},
			
			search: function() {
				$('#AvailableItems').load(this.PageID(), this.Template(), this.find(':input').serialize());
			}
		});

		$('#AvailableItemsFilter :input').concrete({
			onchange: function() {
				$('#AvailableItemsFilter').search();
			}
		});
		
		$('#AvailableItems').concrete({
			load: function(pageid, template, searchterms) {
				this.html('<div class="loading">Loading...</div>');
				this.concrete('.').load(promo_url+'/AvailableItems/'+pageid+'/'+template, searchterms);
			}
		});
	    ...
	    **********************************/
		...
		$('.PromoSection').concrete({
			load_available_items: function(){
				$('#AvailableItemsFilter').load($('#PromoPageSections').d().pageID, this.d().templateClass);
			},
...
jQuery(function($){
	$('#PromoControls').concrete('ss.promospots').init();
	$('#PageID').concrete('ss.promospots').init();
	
	// Handle resizing
	$('#right')
		.bind('resize', function(){ $('#PromoAvailableSections, #PromoPageSections').concrete('ss.promospots').resize() })
		.trigger('resize');
	
	$('.action[value=Save]').bind('click', function(){ $('#PromoPageSections').concrete('ss.promospots').save(); });
});
